// Local script to fetch e-bike ride data from Strava and write static JSON files.
// Usage:
//   1. Create a .env file with STRAVA_CLIENT_ID and STRAVA_CLIENT_SECRET
//   2. Run: node fetch-ebike-rides.js
//   3. Visit http://localhost:3001/auth/strava to authenticate (first time only)
//   4. E-bike rides will be fetched and saved to public/data/
...
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    console.log('Authenticated! Fetching e-bike rides...');
    await fetchEbikeRides();
    res.send('Done! E-bike rides fetched and saved. You can close this tab.');
  } catch (err) {
...
async function fetchEbikeRides() {
  const token = await getAccessToken();
  if (!token) {
    console.log('Not authenticated. Visit http://localhost:3001/auth/strava');
    return;
  }
  ...
  while (true) {
    ...
    for (const a of data) {
      if ((a.type === 'EBikeRide' || a.sport_type === 'EBikeRide') && a.map?.summary_polyline) {
        rides.push(a);
      }
    }
  ...
  console.log(`Found ${rides.length} e-bike rides, fetching detailed polylines...`);
  ...
  fs.writeFileSync(path.join(DATA_DIR, 'ebike-rides.json'), JSON.stringify(geojson));
  console.log(`Done: ${features.length} e-bike rides saved to public/data/ebike-rides.json`);
  process.exit(0);
}

const server = app.listen(PORT, async () => {
  console.log(`E-bike fetch server running at http://localhost:${PORT}`);

  console.log('Visit http://localhost:3001/auth/strava to authenticate and fetch e-bike rides.');
});
