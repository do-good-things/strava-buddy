...
async function runRefresh({ privateStore, publicStore, read, generateRegions, download, full = false, now = Date.now }) {
  ...
  try {
    const result = await syncRides({
      ...
      onFeature: async feature => {
        ...
        feature.properties.photos = await syncPhotos(feature, { read, store: publicStore, download, now });
        await checkpoint();
      },
    });
