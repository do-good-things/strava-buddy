...
async function accessToken({ store, axios, env = process.env, now = Date.now, force = false }) {
  ...
  if (!force && tokens.access_token && tokens.expires_at > Math.floor(now() / 1000) + 60) return tokens.access_token;
  // An operator can replace a revoked Railway secret without deleting the
  // worker volume. A forced rotation is the explicit recovery path, so prefer
  // the configured bootstrap token over the persisted one in that case.
  if (force && env.STRAVA_REFRESH_TOKEN) tokens.refresh_token = env.STRAVA_REFRESH_TOKEN;
  let response;
  try {
